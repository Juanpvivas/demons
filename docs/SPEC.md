...
- ✅ Gap crítico de derrota alcanzable del `Soldado` (identificado en T043, ver historial abajo): cerrado por T050-T053 de `001-soldado-defensor-oleadas`, la alternativa 1 de las esbozadas por `qa-validator` ("agregar una tarea de salud/daño de `Soldado`"), elegida por la persona dueña del proyecto. `UnitStats.max_health` (T050) le da al `Soldado` una salud runtime; `Soldado.take_damage(amount) -> bool` (mismo contrato que `Enemigo.take_damage()`) reduce esa salud y, al llegar a 0, pone la moral acumulada en 0, emite `soldier_defeated(grid_cell)` y hace `queue_free()` (T052) — con cobertura de tests del Edge Case completo (T051). El daño entrante de cada `Enemigo` (`EnemyStats.melee_damage`, antes sin ningún uso) ahora sí se aplica, y con eso el `Soldado` deja de ser estructuralmente indestructible: al ser derrotado libera su celda y su cuerpo físico, dejando de bloquear permanentemente el carril, con lo que la condición de derrota de FR-013 vuelve a ser alcanzable. La forma concreta en la que se detecta ese daño entrante (T053, con una vuelta atrás incluida) dejó un hallazgo de motor de alcance de proyecto, graduado a `docs/ARCHITECTURE.md` §4.2: una `Area2D` hija de un `CharacterBody2D` que se mueve con `move_and_slide()` (como `Enemigo`) no detecta de forma fiable sus overlaps contra un `StaticBody2D` (como `Soldado`) — por eso el daño se dispara desde la `Area2D` "RangoMelee" del propio `Soldado`, no desde una `Area2D` de `Enemigo`. No se tocó `collision_layer`/`collision_mask` (alternativa 2) ni la distribución de `wave_spawn_position` (alternativa 3) — ambas quedaron descartadas para este gap en favor de la alternativa 1. (spec `001-soldado-defensor-oleadas`)
