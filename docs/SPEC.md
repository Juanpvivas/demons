...
- **Gap crítico identificado durante T043 de `001-soldado-defensor-oleadas`** (confirmado de forma independiente y empírica por `godot-tester` y `qa-validator`, corriendo tests con física real, no solo lectura de código): `Soldado` (`StaticBody2D`) y `Enemigo` (`CharacterBody2D`) no declaran `collision_layer`/`collision_mask` explícitos en ninguna escena (`Soldado.tscn`, `EnemigoBase.tscn`) — ambos quedan en el default de Godot (layer 1 / mask 1), por lo que colisionan físicamente entre sí. Un `Enemigo` que avanza con `move_and_slide()` hacia un `Soldado` vivo en su mismo carril queda bloqueado físicamente ANTES de poder alcanzar la `Area2D` "PosicionDefendida" (T043), ubicada más allá del soldado en ese carril. Además, `Soldado` no tiene, en ninguna tarea planeada hoy en `tasks.md` (T001-T049 revisadas completas), ninguna forma de ser derrotado: no tiene salud, no recibe daño de `Enemigo`, y no emite `soldier_defeated` — a pesar de que esa señal SÍ está contratada en `contracts/signals.md` ("El soldado es derrotado — pierde moral, libera celda") y el Edge Case correspondiente está descrito explícitamente en `spec.md` línea 76 de esa feature. Además, `EnemyStats.melee_damage` (T005) está definido pero nunca leído en ningún lugar del código (`grep` confirma cero usos fuera de la declaración). **Consecuencia:** la condición de derrota de FR-013 / Acceptance Scenario 3 de User Story 3 (`spec.md` línea 66: "un enemigo alcanza la posición defendida... termina en derrota") es, en la práctica, estructuralmente inalcanzable con el conjunto de tareas planeado hoy — mientras el soldado inicial de un carril exista (y no puede morir), ningún enemigo de ese carril puede pasarlo físicamente para llegar a "PosicionDefendida". Esto también le quita sentido práctico a la dificultad creciente de las oleadas (SC-005): solo alargaría el tiempo de juego, sin riesgo real de derrota. **Pendiente de decisión humana antes de continuar con T044** — no resuelto en este documento. Alternativas esbozadas por `qa-validator` durante la validación de T043, sin elegir ninguna:
  1. Agregar una tarea de salud/daño de `Soldado` que consuma `EnemyStats.melee_damage` (hoy sin ningún uso) y emita `soldier_defeated` al llegar a cero.
  2. Ajustar `collision_layer`/`collision_mask` de `Soldado`/`Enemigo` para que el enemigo no quede bloqueado físicamente por el soldado (dejaría de haber colisión física entre ambos tipos de cuerpo).
  3. Redistribuir `wave_spawn_position` (hoy un único punto para todas las oleadas, `nivel_monte_calvo.gd`) a múltiples carriles, para que no todo enemigo dependa de pasar por el mismo soldado.
